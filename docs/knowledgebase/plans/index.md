...
* [Build system](build-system.md) - a `nemo build` command with a `nemo.toml` manifest, compiled `.nemo` component artifacts, opt-in `dist/` project builds, and Go-style remote component libraries; expands and supersedes SFC "Phase 5". Phases 0–2 implemented (manifest, project-root discovery, manifest-aware launch; `nemo build` compiles `.nemo` files / `[package]` libraries to JSON artifacts and builds an app project to a loadable `dist/`; `--dist` loads it); Phase 3 (remote packages) pending.
