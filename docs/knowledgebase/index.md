...
* [Build system](plans/build-system.md) - a `nemo build` command with a `nemo.toml` manifest, compiled `.nemo` component artifacts, opt-in `dist/` project builds, and Go-style remote component libraries in `.nemo/packages`; expands and supersedes SFC "Phase 5". **Phases 0–1 implemented** (`nemo.toml` manifest, project-root discovery, manifest-aware launch; `nemo build` compiles a `.nemo` file or `[package]` library to JSON component artifacts); Phases 2–3 pending.
